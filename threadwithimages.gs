...
var POST_URL = "DiscordWebhookURL";
...
function onSubmit(e) {
  ...
  for (var i = 0; i < response.length; i++) {
    ...
    if (question == "Location of Base"){ // Makes Thread Title which is needed can be any answer if want
      locationname = answer;
    }
  ...
  if(thread_id == null){
    thread_id = sendEmbed(items, locationname, imagesnew);
  } else{
...
function sendEmbed(items, locationname, imagesnew){
  var stringer = "";
  ...
  var options = {
    ...
    "payload": JSON.stringify({
        "thread_name" : locationname,
        "embeds": [{
            "title": "Excessive Build Report",
